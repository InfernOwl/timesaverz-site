...
import { createGameStoreLink } from "../graphql/mutations";
const client = generateClient();
export default function GameStoreLinkCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    setSteam(initialValues.steam);
    setEpic(initialValues.epic);
    setPlaystation(initialValues.playstation);
    setXbox(initialValues.xbox);
    setNintendo(initialValues.nintendo);
    setErrors({});
  };
  const validations = {
    steam: [{ type: "URL" }],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: createGameStoreLink.replaceAll("__typename", ""),
            variables: {
              input: {
                ...modelFields,
              },
      ...
      {...getOverrideProps(overrides, "GameStoreLinkCreateForm")}
      {...rest}
    >
      <TextField
        label="Steam"
